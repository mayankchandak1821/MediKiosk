    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <div className="bg-white border border-[#E2DCBE] rounded-2xl p-6 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-[#59C749] flex items-center justify-center font-black text-white text-2xl shadow-sm">
            {patientProfile.full_name.charAt(0)}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-extrabold text-xl text-[#142618]">{patientProfile.full_name}</h2>
              <span className="px-2.5 py-0.5 rounded-full bg-[#59C749]/15 border border-[#59C749]/30 text-[#142618] font-mono text-xs font-bold flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-[#59C749]" /> ABDM VERIFIED
              </span>
            </div>
            <p className="text-xs text-[#526857] font-mono mt-0.5">
              ABHA ID: {patientProfile.abha_id} • {patientProfile.gender}, {patientProfile.age} Yrs • Phone: {patientProfile.phone}
            </p>
          </div>
        </div>

        {/* 2 Side-by-Side Action Buttons: Describe Illness & OCR Detection */}
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-white hover:bg-[#F7F4E1] text-[#142618] border border-[#E2DCBE] font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center gap-2 cursor-pointer"
          >
            <ScanLine className="w-4 h-4 text-[#59C749]" /> OCR Detection
          </button>
        </div>
      </div>
      ...
      {showTreeModal && (
        <div className="bg-white border-2 border-[#59C749]/40 rounded-3xl p-5 md:p-6 shadow-md space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#E2DCBE] pb-3">
            <span className="text-xs font-extrabold text-[#142618] uppercase tracking-wider flex items-center gap-2">
              <Zap className="w-4 h-4 text-[#59C749]" /> Interactive Clinical Symptom Decision Tree
            </span>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg font-bold text-[11px] transition-all border cursor-pointer ${
                    treeCategory === cat.id
                      ...
                      : 'bg-[#FFFDF1] border-[#E2DCBE] text-[#526857] hover:bg-[#F7F4E1]'
                  }`}
                >
                  {cat.label}
                </button>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="p-5 rounded-2xl bg-[#FFFDF1] border border-[#E2DCBE] shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-[#E2DCBE] pb-3">
              <span className="text-xs font-extrabold text-[#142618] uppercase tracking-wider flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-[#59C749]" /> Ayushman Bharat Health Card
              </span>
              <span className="px-2 py-0.5 rounded bg-[#59C749]/15 text-[#142618] font-mono text-[10px] font-bold">
                ABDM PHR
              </span>
            </div>
            ...
            <div className="space-y-2.5 text-xs font-mono">
              <div className="flex justify-between">
                <span className="text-[#526857]">Full Name:</span>
                <span className="text-[#142618] font-bold">{patientProfile.full_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#526857]">ABHA Number:</span>
                <span className="text-[#2B8A1E] font-bold">{patientProfile.abha_id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#526857]">Age / Gender:</span>
                <span className="text-[#142618]">{patientProfile.age} Yrs / {patientProfile.gender}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#526857]">Blood Group:</span>
                <span className="text-rose-700 font-bold">{patientProfile.blood_group}</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#E2DCBE] shadow-xs space-y-3">
            <h3 className="text-xs font-bold text-[#526857] uppercase tracking-wider flex items-center gap-2">
              <Activity className="w-4 h-4 text-[#59C749]" /> Current Kiosk Sensor Vitals
            </h3>
            <div className="grid grid-cols-3 gap-2.5 font-mono text-center text-xs">
              <div className="p-3 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE]">
                <span className="text-[#526857] block text-[10px]">Temp</span>
                <span className="font-bold text-amber-800 text-sm">{currentVitals.temperature_c}°C</span>
              </div>
              <div className="p-3 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE]">
                <span className="text-[#526857] block text-[10px]">SpO2</span>
                <span className="font-bold text-[#2B8A1E] text-sm">{currentVitals.spo2_percent}%</span>
              </div>
              <div className="p-3 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE]">
                <span className="text-[#526857] block text-[10px]">Heart Rate</span>
                <span className="font-bold text-rose-700 text-sm">{currentVitals.heart_rate_bpm} BPM</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-white border border-[#E2DCBE] shadow-xs space-y-3">
            <h3 className="text-xs font-bold text-[#526857] uppercase tracking-wider flex items-center gap-2">
              <Clock className="w-4 h-4 text-[#59C749]" /> My OPD Visit Encounters
            </h3>

            <div className="space-y-2">
              {patientEncounters.length === 0 ? (
                <div className="p-4 text-center text-[#526857] text-xs italic">
                ...
                patientEncounters.map((enc) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="font-bold text-[#142618]">{enc.symptomCategory}</span>
                      <span className="text-[10px] text-[#526857] font-mono">{enc.timestamp}</span>
                    </div>
                    <div className="flex items-center justify-between text-[11px] text-[#526857]">
                      <span>{enc.answers?.site || 'Routine Evaluation'}</span>
                      {enc.status === 'COMPLETED_SIGNED_OFF' ? (
                        <span className="px-2 py-0.5 rounded bg-[#59C749]/20 text-[#2B8A1E] text-[10px] font-bold">
                          ✅ SIGNED OFF
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 rounded bg-[#FFFDF1] border border-[#E2DCBE] text-[#142618] text-[10px] font-bold">
                          QUEUED
                        </span>
                      )}
                    </div>
          ...
          {activeEnc ? (
            <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#E2DCBE] pb-4">
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-extrabold text-lg text-[#142618]">{activeEnc.symptomCategory} OPD Visit</h3>
                    <span className="px-2.5 py-0.5 rounded-full bg-[#59C749]/15 text-[#142618] font-mono text-xs font-bold border border-[#59C749]/30">
                      {activeEnc.id}
                    </span>
                  </div>
                  <p className="text-xs text-[#526857] font-mono mt-0.5">Visit Date: {activeEnc.timestamp} • Care Mode: {activeEnc.careMode}</p>
                </div>
                ...
                >
                  <Download className="w-4 h-4 text-[#59C749]" /> Download Health Summary
                </button>
              </div>
              ...
              {activeEnc.status === 'COMPLETED_SIGNED_OFF' ? (
                <div className="p-4 rounded-2xl bg-[#59C749]/10 border border-[#59C749]/40 text-[#142618] flex items-center justify-between text-xs shadow-xs">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="font-bold text-[#142618] text-sm block">Physician Evaluation Signed Off & Discharged</span>
                      <span className="text-[#526857]">Doctor Notes Attached • Official Prescription Ready</span>
                    </div>
                  </div>
                  <button
                    onClick={() => downloadPatientRecord(activeEnc)}
                ...
                <div className="p-4 rounded-2xl bg-[#FFFDF1] border border-[#E2DCBE] text-[#142618] flex items-center gap-3 text-xs">
                  <Clock className="w-5 h-5 text-[#59C749] shrink-0" />
                  <div>
                    <span className="font-bold text-[#142618] text-sm block">Encounter Submitted to OPD Doctor Queue</span>
                    <span className="text-[#526857]">Patient is currently waiting for physician consultation in OPD queue.</span>
                  </div>
                </div>
              )}

              {/* Extracted Past History & Active Medications */}
              <div className="p-5 rounded-2xl bg-[#FFFDF1] border border-[#E2DCBE] space-y-4">
                <div className="flex items-center justify-between border-b border-[#E2DCBE] pb-3">
                  <span className="font-extrabold text-sm text-[#142618] flex items-center gap-2">
                    <FileText className="w-4 h-4 text-[#59C749]" /> Prescription & Past History Entities
                  </span>
                  <span className="text-[10px] font-mono text-[#526857] font-bold">OCR DIGITIZED</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  {/* Past Diagnoses */}
                  <div className="p-3 rounded-xl bg-white border border-[#E2DCBE] space-y-2">
                    <span className="text-[10px] font-bold text-[#526857] uppercase tracking-wider block">Past Medical Diagnoses</span>
                    <div className="flex flex-wrap gap-1">
                      {(activeEnc.scannedDoc?.pastMedicalHistory || activeEnc.scannedDoc?.diagnoses || []).length > 0 ? (
                        ...
                        <span className="text-[#526857] italic">No past chronic condition documented</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-white border border-[#E2DCBE] space-y-2">
                    <span className="text-[10px] font-bold text-[#526857] uppercase tracking-wider block">Prescribed Active Medications</span>
                    <div className="space-y-1">
                      {activeEnc.scannedDoc?.medications?.length > 0 ? (
                        activeEnc.scannedDoc.medications.map((m, i) => (
                          <div key={i} className="flex items-center justify-between font-mono text-[11px] bg-[#FFFDF1] border border-[#E2DCBE] px-2 py-1 rounded">
                            <span className="text-[#142618] font-bold">💊 {m.name} {m.dosage}</span>
                            <span className="text-[#2B8A1E] font-semibold">{m.frequency}</span>
                          </div>
                        ))
                      ) : (
                        <span className="text-[#526857] italic">No active medications extracted</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="p-4 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE] space-y-2 font-mono">
                  <span className="text-[10px] font-bold text-[#526857] uppercase tracking-wider block">Intake Symptoms</span>
                  <p className="text-[#526857]">Category: <span className="text-[#142618] font-bold">{activeEnc.symptomCategory}</span></p>
                  <p className="text-[#526857]">Location: <span className="text-[#142618]">{activeEnc.answers?.site || 'General'}</span></p>
                  <p className="text-[#526857]">Discomfort: <span className="text-[#142618]">{activeEnc.answers?.character || 'Routine Evaluation'}</span></p>
                  <p className="text-[#526857]">Pain Scale: <span className="text-amber-800 font-bold">{activeEnc.answers?.severity || 1}/10</span></p>
                </div>

                <div className="p-4 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE] space-y-2 font-mono">
                  <span className="text-[10px] font-bold text-[#526857] uppercase tracking-wider block">Telemetry Vital Signs</span>
                  <p className="text-[#526857]">Temperature: <span className="text-amber-800 font-bold">{activeEnc.vitals?.temperature_c || 37.0}°C</span></p>
                  <p className="text-[#526857]">Oxygen (SpO2): <span className="text-[#2B8A1E] font-bold">{activeEnc.vitals?.spo2_percent || 98}%</span></p>
                  <p className="text-[#526857]">Heart Rate: <span className="text-rose-700 font-bold">{activeEnc.vitals?.heart_rate_bpm || 72} BPM</span></p>
                  <p className="text-[#526857]">Triage Risk: <span className="text-[#2B8A1E] font-bold">{activeEnc.triage?.riskPercentage || 15}% ROUTINE</span></p>
                </div>
              </div>
              ...
              {activeEnc.decisionTreeEval && (
                <div className="p-5 rounded-2xl bg-white border border-[#E2DCBE] shadow-xs space-y-4">
                  <div className="flex items-center justify-between border-b border-[#E2DCBE] pb-3">
                    ...
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-[#59C749]/15 border border-[#59C749]/30 text-[#142618] font-bold uppercase">
                    ...
                    {activeEnc.decisionTreeEval.differentials?.map((diff, i) => (
                      <div key={i} className="p-3 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE] flex items-center justify-between gap-3 text-xs font-mono">
                        <div>
                          ...
                          <span className="text-[10px] text-[#526857]">Action Recommendation: {diff.action}</span>
                        </div>
                  ...
                  {activeEnc.decisionTreeEval.recommendation && (
                    <div className="p-3 rounded-xl bg-[#59C749]/10 border border-[#59C749]/30 text-xs text-[#142618] font-medium">
              ...
              {activeEnc.doctor_notes && (
                <div className="p-4 rounded-xl bg-[#FFFDF1] border border-[#E2DCBE] space-y-2">
                  <span className="text-xs font-bold text-[#142618] uppercase tracking-wider block">Physician Assessment & Consultation Notes</span>
                  <p className="text-xs text-[#142618] leading-relaxed font-mono bg-white p-3 rounded-lg border border-[#E2DCBE]">
                    {activeEnc.doctor_notes}
                  </p>
                </div>
              )}
            ...
            <div className="p-12 text-center text-[#526857] space-y-2">
              <User className="w-10 h-10 text-[#59C749]/40 mx-auto" />
              <p className="text-sm font-bold text-[#142618]">No Patient Encounter Selected</p>
              <p className="text-xs text-[#526857]">Click "Describe Illness (Decision Tree)" above to evaluate your symptoms.</p>
            </div>
          )}
        </div>
      </div>
